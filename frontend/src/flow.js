// src/flow.js - FINAL COMPLETE VERSION WITH ALL FIXES
// Nuclear dropdowns with proper positioning, exact width matching, and layout stability

// NUCLEAR DROPDOWN IMPLEMENTATION - All fixes applied
function initDropdownsNuclear() {
    console.log('🚀 NUCLEAR DROPDOWN INIT - Complete JS override with all fixes');
...
function createNuclearOptionsContainer(dropdown, originalOptions, originalOptionElements) {
    ...
    // Add to body for absolute positioning relative to document
    document.body.appendChild(nuclearOptions);
    
    return nuclearOptions;
}

// FIXED: Positioning with exact width matching
function showNuclearOptions(nuclearOptions, dropdown) {
    ...
    // FIXED: Get the exact computed width including borders and padding
    const exactWidth = displayRect.width;
    
    console.log(`🔧 Button width: ${exactWidth}px`);
    ...
    // FIXED: Set exact width first, then show to measure height
    nuclearOptions.style.width = Math.round(exactWidth) + 'px';
    nuclearOptions.style.minWidth = Math.round(exactWidth) + 'px';
    nuclearOptions.style.maxWidth = Math.round(exactWidth) + 'px';
    ...
    nuclearOptions.style.left = Math.round(left) + 'px';
    nuclearOptions.style.top = Math.round(top) + 'px';
    
    console.log(`🔽 Nuclear options positioned at: ${Math.round(left)}, ${Math.round(top)} (exact width: ${Math.round(exactWidth)}px, above: ${isPositionedAbove})`);
    ...
    nuclearOptions.style.position = 'absolute';
}
...
// Better download dropdown with enhanced positioning
...
// FIXED: Download dropdown positioning with exact width
function showNuclearDownloadOptions(nuclearOptions, triggerElement) {
    ...
    // FIXED: Get exact width including all styling
    const exactWidth = triggerRect.width;
    
    console.log(`🔧 Download button width: ${exactWidth}px`);
    ...
    let top = triggerRect.top + scrollY - dropdownHeight - 8; // 8px gap above
    
    // If doesn't fit above, position below
    if (triggerRect.top - dropdownHeight < 10) {
        top = triggerRect.bottom + scrollY + 8; // 8px gap below
    ...
    console.log(`🔽 Download options positioned at: ${Math.round(left)}, ${Math.round(top)} (exact width: ${Math.round(exactWidth)}px, above: ${isPositionedAbove})`);
}
...
function updateDropdownDisabledState() {
  ...
  // Update polycount controls disabled state
  ...
  // Update PBR checkbox disabled state
  const pbrCheckbox = document.getElementById('pbrCheckbox');
  if (pbrCheckbox) {
    pbrCheckbox.disabled = isDisabled;
  }
}
...
function hidePBRControls() {
  ...
  if (pbrButtons) {
    pbrButtons.classList.add('hidden');
  }
...
document.addEventListener('DOMContentLoaded', function() {
  ...
  if (document.querySelector('.controls-sidebar')) {
    console.log("🔄 Initializing flow with FINAL NUCLEAR DROPDOWNS...");
    ...
    console.log('🧪 FINAL NUCLEAR DROPDOWN test functions available:');
